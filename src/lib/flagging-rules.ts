...
export function buildPublicSafetyAlert(input: {
  ...
  if (input.isOwner) {
    if (suspended) {
      return {
        ...
        messages: [
          "RentMe hid this listing from search after reports or a safety review.",
  ...
  if (input.isFlagged) {
    return {
      ...
      messages: [
        ...
        "Keep the conversation on RentMe so there is a record if something goes wrong.",
      ],
      hideDirectContact: true,
      blockInquiries: false,
    };
  ...
  if (signals.length >= 2) {
    return {
      ...
      messages: [
        ...signals.slice(0, 3),
        "Visit in person before paying, and use RentMe messages instead of off-platform payment requests.",
      ],
      hideDirectContact: false,
      blockInquiries: false,
    };
