...
export default function PropertyDetailClient({
  property,
  isOwner,
  alreadyReported: initialAlreadyReported,
  userId,
  ...
  const [alreadyReported, setAlreadyReported] = useState(initialAlreadyReported);
  ...
  const location = [property.neighborhood, property.district, property.city, "Uganda"]
    .filter(Boolean)
    .join(", ");
  const fees: FeeBreakdown | null = property.rent
    ...
    <MainLayout>
      ...
      <div className="border-b border-gray-100 bg-white">
        <div className="page-container py-3">
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span>/</span>
            <Link href="/search" className="hover:text-brand-600" onClick={(e) => {
      ...
      <div className="page-container py-6">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2">
            ...
            {/* Property Info */}
            <div className="mt-6">
              <ScamAlert level={safety.level} title={safety.title} messages={safety.messages} className="mb-6" />
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    title={isOwner ? "You cannot report your own listing" : alreadyReported ? "You already reported this listing" : "Report this listing"}
                  >
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    {videos.map((video: any) => (
                      <div key={video.id} className="overflow-hidden rounded-xl bg-gray-100">
                        <video src={video.url} controls preload="metadata" className="aspect-video w-full object-cover" />
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="mt-3 rounded-xl border border-gray-200 p-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-lg font-bold text-brand-600">{formatUGX(fees.totalMoveInCost)}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-8">
                ...
                <p className="mt-3 whitespace-pre-line text-gray-600 leading-relaxed">{property.description}</p>
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-6">
              ...
              <div className="card p-6">
                <div className="mb-4">
                  <p className="text-3xl font-bold text-brand-600 font-display">{formatUGX(property.rent)}</p>
                ...
                {applyResult && (
                  <div className={`mb-3 rounded-lg px-4 py-3 text-sm ${applyResult.ok ? "bg-green-50 text-green-800" : "bg-amber-50 text-amber-800"}`}>
                    <div className="flex items-start gap-2">
                      {applyResult.ok ? <CheckCircle className="mt-0.5 h-4 w-4 shrink-0" /> : <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />}
                      <div>
                        <p>{applyResult.message}</p>
                        {applyResult.ok && (
                          <a href="/dashboard/tenant/applications" className="mt-1 inline-block font-semibold underline">
                  ...
                  <>
                    ...
                    {applyOpen && (
                      <div className="mt-3 space-y-3 rounded-lg border border-gray-200 bg-gray-50 p-4">
                        <div>
                          <label className="block text-xs font-medium text-gray-600">Preferred move-in date (optional)</label>
                          <input type="date" value={applyMoveIn} min={new Date().toISOString().slice(0, 10)} onChange={(e) => setApplyMoveIn(e.target.value)} className="input mt-1" />
                        </div>
                        <div>
                          <label className="block text-xs font-medium text-gray-600">Message to the landlord (optional)</label>
                          <textarea value={applyMessage} onChange={(e) => setApplyMessage(e.target.value)} rows={2} maxLength={500} placeholder="Tell the landlord about yourself…" className="input mt-1 resize-none" />
                        </div>
                        <button onClick={handleApply} disabled={applySubmitting || inquiriesBlocked} className="btn-primary w-full">
                ...
                <button onClick={() => setShowViewingModal(true)} className="btn-secondary mt-3 w-full" disabled={inquiriesBlocked}>
                  <Calendar className="mr-2 h-4 w-4" />
                  Request Viewing
                </button>

                <button onClick={() => setShowContactModal(true)} className="btn-secondary mt-3 w-full" disabled={inquiriesBlocked}>
                  <MessageSquare className="mr-2 h-4 w-4" />
                  Contact Landlord
                </button>

                {!hidePhone && property.user.phone && (
                  <a href={`tel:${property.user.phone}`} className="btn-secondary mt-3 w-full">
              ...
              <div className="card p-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {property.user.landlord?.verificationStatus === "VERIFIED" || property.user.agent?.verificationStatus === "VERIFIED" ? (
      ...
      {galleryOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90">
          <button onClick={() => setGalleryOpen(false)} className="absolute right-4 top-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20">
            <X className="h-5 w-5" />
          </button>
      ...
      {showContactModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="card w-full max-w-md p-6">
            ...
            <MessageLandlord className="mt-4" propertyId={property.id} landlordId={property.user.id} landlordName={property.user.name} label="Send Message" />
            <div className="mt-3 flex">
              <button onClick={() => setShowContactModal(false)} className="btn-secondary flex-1">Cancel</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="card w-full max-w-md p-6">
            ...
            <div className="mt-4 space-y-3">
              <div>
                ...
                <input type="date" value={viewingDate} onChange={(e) => setViewingDate(e.target.value)} className="input" min={new Date().toISOString().split("T")[0]} />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" value={viewingPeople} onChange={(e) => setViewingPeople(parseInt(e.target.value) || 1)} className="input" min="1" max="10" />
              </div>
              <div>
                <label className="label">Message (optional)</label>
                <textarea value={viewingMessage} onChange={(e) => setViewingMessage(e.target.value)} className="input" rows={3} placeholder="Any special requests?" />
              </div>
            </div>
            <div className="mt-4 flex gap-3">
              <button onClick={() => setShowViewingModal(false)} className="btn-secondary flex-1">Cancel</button>
              <button onClick={handleViewingRequest} className="btn-primary flex-1">Send Request</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showReportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="card w-full max-w-md p-6">
            ...
            <p className="mt-1 text-sm text-gray-500">Flags are reviewed by RentMe. Scam reports also warn other tenants.</p>
            ...
            <textarea
              ...
              placeholder={isReportReason(reportReason) && descriptionRequired(reportReason) ? "Describe what happened (required for scam and safety reports)" : "Additional details (optional)"}
              className="input mt-3"
              rows={3}
            />
            <div className="mt-4 flex gap-3">
              <button onClick={() => setShowReportModal(false)} className="btn-secondary flex-1" disabled={reporting}>Cancel</button>
